...
const userCtrl = {
    register: async (req, res) => {
        try {
            ...
            if(password.length<6) return res.status(400).json({msg: 'Password will be at least 6 characters long.'});
    ...
    refreshToken: (req, res) => {
        try {
            ...
            if(!rf_token) return res.status(400).json({msg: 'Please Login or Registration.'});

            jwt.verify(rf_token,process.env.REFRESH_TOKEN_SECRET, (err,user) => {
                if(err) return res.status(400).json({msg: 'Please Login or Registration.'});
